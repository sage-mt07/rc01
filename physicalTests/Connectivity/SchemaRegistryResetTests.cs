...
public class SchemaRegistryResetTests
{
    ...
    public async Task Setup_ShouldRegisterAllSchemas()
    {
        ...
        var subjects = await Http.GetFromJsonAsync<string[]>("http://localhost:8088/subjects");
    ...
    public async Task DuplicateSchemaRegistration_ShouldSucceed()
    {
        ...
        var latest = await Http.GetFromJsonAsync<JsonElement>("http://localhost:8088/subjects/orders-value/versions/latest");
        var schema = latest.GetProperty("schema").GetString();
        var resp = await Http.PostAsJsonAsync("http://localhost:8088/subjects/orders-value/versions", new { schema });
